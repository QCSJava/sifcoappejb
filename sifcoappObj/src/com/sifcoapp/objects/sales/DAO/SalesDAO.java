...
public class SalesDAO extends CommonDAO {
	...
	public List getSales(SalesInTO param) throws Exception {
		...
		this.setTypeReturn(Common.TYPERETURN_CURSOR);
		// (_docnum integer, _docdate date, _series integer, _towhscode
		// character varying, _ref1 character varying, _ref2 character varying,
		// _comments character varying, _docduedate date, _cardcode character
		// varying, _cardname character varying, _taxdate date, _doctype
		// character, _peymethod character varying, _docstatus character)
		...
		if (param.getTaxdate() == null) {
			...
			java.sql.Date fecha = new java.sql.Date(param.getTaxdate()
					.getTime());
		...
		while (liRowset.hasNext()) {
			...
			try {
				while (rowsetActual.next()) {
					...
					sales.setFquedan(rowsetActual.getDate(41));
					sales.setUsersign(rowsetActual.getInt(42));
					sales.setSalesDetails(Detail.getSalesDetail(rowsetActual
							.getInt(1)));
					_return = sales;
				}
				rowsetActual.close();
	...
	public int inv_Sales_mtto(SalesTO parameters, int accion) throws Exception {
		...
		if (parameters.getPaidtodate() == null) {
			...
			java.sql.Date fecha = new java.sql.Date(parameters.getPaidtodate()
					.getTime());
		...
		if (parameters.getCanceldate() == null) {
			...
			java.sql.Date fecha = new java.sql.Date(parameters.getCanceldate()
					.getTime());
		...
		if (parameters.getFechreciva() == null) {
			...
			java.sql.Date fecha = new java.sql.Date(parameters.getFechreciva()
					.getTime());
		...
		if (parameters.getFquedan() == null) {
			...
			java.sql.Date fecha = new java.sql.Date(parameters.getFquedan()
					.getTime());
