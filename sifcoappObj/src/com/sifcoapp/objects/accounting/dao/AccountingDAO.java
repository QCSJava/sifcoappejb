...
public class AccountingDAO extends CommonDAO {
	...
	public int update_endTotal(AccountTO transaction) throws Exception {
		...
		if (transaction.getGroupmask() == 2 || transaction.getGroupmask() == 3
				|| transaction.getGroupmask() == 5) {
	...
	public void update_TreeTotal_date(Date Refdate, String transtype)
		...
		if (Refdate == null) {
			this.setDate(1, "_refdate", Refdate);
		} else {
			java.sql.Date fecha = new java.sql.Date(Refdate.getTime());
			this.setDate(1, "_refdate", fecha);
		}

		this.setString(2, "_filter", transtype); // para devolver todas las
													// cuentas
